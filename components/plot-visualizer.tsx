...
export function PlotVisualizer({
  ...
  const svgW = plotW + PAD * 2 + 28  // extra right margin for aisle label
  const svgH = plotH + PAD * 2

  const spacingXpx = spacingInRow * PX_PER_IN
  const spacingYpx = rowSpacing * PX_PER_IN
  const defaultBedWidth = bedWidth ?? rowSpacing * 0.55
  const defaultBedHeightPx = defaultBedWidth * PX_PER_IN
  ...
  function bedHeightForRow(rowIdx: number): number {
    const sIdx = sectionIndexForRow(rowIdx)
    const section = cropSections?.[sIdx]
    return section ? section.bedWidth * PX_PER_IN : defaultBedHeightPx
  }
  ...
  const aisleWidthIn = Math.max(0, rowSpacing - defaultBedWidth)
  ...
  return (
    ...
    >
      ...
      {/* Bed strips — per-section bed width */}
      {rowYs.map((ry, i) => {
        const bh = bedHeightForRow(i)
        return (
          <rect
            key={i}
            x={PAD}
      ...
      {plantsInFirstRow >= 2 && (
        <g>
          ...
          <text x={(plants[0].x + plants[1].x) / 2} y={plants[0].y + 22} textAnchor="middle" fontSize={11} fontWeight="600" fill="#c0562a">
            {spacingInRow}&quot;
          </text>
        </g>
      )}
      ...
      {rowYs.length > 0 && (
        <g>
          <line x1={PAD - 6} y1={PAD + rowYs[0] - defaultBedHeightPx / 2} x2={PAD - 6} y2={PAD + rowYs[0] + defaultBedHeightPx / 2} stroke="#4a7c59" strokeWidth={1} />
          <line x1={PAD - 9} y1={PAD + rowYs[0] - defaultBedHeightPx / 2} x2={PAD - 3} y2={PAD + rowYs[0] - defaultBedHeightPx / 2} stroke="#4a7c59" strokeWidth={1} />
          <line x1={PAD - 9} y1={PAD + rowYs[0] + defaultBedHeightPx / 2} x2={PAD - 3} y2={PAD + rowYs[0] + defaultBedHeightPx / 2} stroke="#4a7c59" strokeWidth={1} />
          <text
            x={PAD - 18} y={PAD + rowYs[0] + 4}
            textAnchor="middle" fontSize={10} fontWeight="600" fill="#4a7c59"
            transform={`rotate(-90, ${PAD - 18}, ${PAD + rowYs[0]})`}
          >
            {Math.round(defaultBedWidth)}&quot; bed
          </text>
        </g>
      )}
      ...
      {rowYs.length >= 2 && aisleWidthIn > 0 && (
        <g>
          <line x1={PAD + plotW + 6} y1={PAD + rowYs[0] + defaultBedHeightPx / 2} x2={PAD + plotW + 6} y2={PAD + rowYs[1] - defaultBedHeightPx / 2} stroke="#6b4c28" strokeWidth={1} />
          <line x1={PAD + plotW + 3} y1={PAD + rowYs[0] + defaultBedHeightPx / 2} x2={PAD + plotW + 9} y2={PAD + rowYs[0] + defaultBedHeightPx / 2} stroke="#6b4c28" strokeWidth={1} />
          <line x1={PAD + plotW + 3} y1={PAD + rowYs[1] - defaultBedHeightPx / 2} x2={PAD + plotW + 9} y2={PAD + rowYs[1] - defaultBedHeightPx / 2} stroke="#6b4c28" strokeWidth={1} />
          <text
            x={PAD + plotW + 20}
            y={(PAD + rowYs[0] + defaultBedHeightPx / 2 + PAD + rowYs[1] - defaultBedHeightPx / 2) / 2 + 4}
            textAnchor="middle" fontSize={10} fontWeight="600" fill="#6b4c28"
            transform={`rotate(-90, ${PAD + plotW + 20}, ${(PAD + rowYs[0] + defaultBedHeightPx / 2 + PAD + rowYs[1] - defaultBedHeightPx / 2) / 2})`}
          >
      ...
      <text x={PAD + plotW / 2} y={PAD - 16} textAnchor="middle" fontSize={13} fontWeight="600" fill="#444">{plotWidth} ft</text>
      ...
      <text
        x={PAD - 24} y={PAD + plotH / 2}
        textAnchor="middle" fontSize={13} fontWeight="600" fill="#444"
